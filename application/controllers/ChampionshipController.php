...
class ChampionshipController extends App_Controller_FirstBootController
{
    ...
    public function addAction()
    {
        ...
        if ($league_data) {
            ...
            $form->setAction(
                $this->view->url(
                    array('controller' => 'championship', 'action' => 'add', 'league_id' => $league_id),
                    'leagueActionAddChamp', true
                )
            );
    ...
    public function editAction()
    {
        ...
        if ($league_data) {
            ...
            if ($championship_data) {
                ...
                $form->setAction(
                    $this->view->url(
                        array('controller' => 'championship', 'action' => 'edit', 'id' => $championship_id),
                        'championship', true
                    )
                );
                $form->cancel->setAttrib(
                    'onClick', "location.href=\"{$this->view->url(
                        array('controller' => 'championship', 'action' => 'id', 'id' => $championship_id),
                        'championship', true
                    )}\""
                );
        ...
        if ($league_data) {
            ...
            if ($championship_data) {
                ...
                if ($championship_team_data) {
                    ...
                    $form->cancel->setAttrib(
                        'onClick', "location.href=\"{$this->view->url(
                            array('controller'      => 'championship', 'action' => 'team-show',
                                  'league_id'       => $league_id, 'championship_id' => $championship_id,
                                  'team_id'         => $team_id), 'championshipTeam', true
                        )}\""
                    );
                            ...
                            if ($form->getValue('logo_team')) {
                                if ($form->logo->receive()) {
                                    ...
                                    $new_championship_team_data['url_logo_car']
                                        = '/img/data/logos/teams/car/' . $newName;

...
class Application_Model_DbTable_ChampionshipTeamDriver extends Zend_Db_Table_Abstract {
	...
	public function getChampionshipTeamDriver($championship_id, $team_id, $user_id) {
		$model = new self;
		...
		$select = $model->select()
				...
				->where('CTD.championship_id = ?', $championship_id)
				->where('CTD.team_id = ?', $team_id)
				->where('CTD.user_id = ?', $user_id)
				->join(array('u' => 'user'), "{$user_id} = u.id", array(
					'user_id' => 'u.id',
					'user_login' => 'u.login',
					'user_name' => 'u.name',
					'user_surname' => 'u.surname',
		...
		$championshipTeamDriver = $model->fetchRow($select);

		if (count($championshipTeamDriver) != 0) {
			return $championshipTeamDriver;
		} else {
			return FALSE;
		}
	}
